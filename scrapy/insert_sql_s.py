...
def insert(url):
    ...
    for it in data_list:
        ...
        if it['type'] == "video":
            ...
            ####################################################数据库插入操作
            conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='Mnbvcx1!', db='cj_kaiyan',
                                   charset='utf8')
            cursor = conn.cursor()
            cursor.execute(
                'insert into caiji_video(title,description,category,playUrl,duration,date,author_name,author_description,cover_feed,cover_detail,cover_blurred,releaseTime,kaiyan_id,url_nomal) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
                (title, description, category, playUrl, duration, date, author_name, author_description, cover_feed,
                 cover_detail, cover_blurred, releaseTime, kaiyan_id, url_nomal))
...
def do(start):
    global num
    for i in range(start, start+70, 20):
        url = 'http://baobab.kaiyanapp.com/api/v4/discovery/hot?start=%s&num=20' % (i)
        print(url)
        insert(url)
        num = i
start=sys.argv[1]
